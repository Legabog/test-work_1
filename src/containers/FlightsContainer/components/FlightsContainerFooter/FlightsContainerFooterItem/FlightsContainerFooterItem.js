import { converterForMonthStroke } from "../../../../../utils/converters/converterForMonthStroke";
import "./FlightsContainerFooterItem.css";

const FlightsContainerFooterItem = (props) => {
  const convertIdToCarrier = (id) => {
    let result = null;

    props.flightsCarriers.map((e) => {
      if (e.CarrierId === id) {
        result = e.Name;
      }
      return null;
  ...
  const likeClickHandler = () => {
    if (props.flightsFavorites.includes(props.quoteId)) {
      console.log(props.flightsFavorites.indexOf(props.quoteId));
      props.deleteFromFlightsFavorites(
        props.flightsFavorites.indexOf(props.quoteId)
      );
    } else {
      props.addToFlightsFavorites(props.quoteId);
    }
  };

  return (
    <div className="flights-container-footer__item-wrapper">
      <div className="flights-container-footer__item">
        <div className="flights-container-footer__item__icon">
          <svg
            width="35"
            height="35"
            viewBox="0 0 35 35"
        ...
        <div className="flights-container-footer__item__info">
          <div className="main-line-info">
            ...
            <div className="main-line-info-second-place">
              <span>&nbsp;New York City (JFK) </span>
            </div>
          </div>
          <div className="date-line-info">
            <span>
              {props.quoteData.slice(8, 10)}&nbsp;
              {converterForMonthStroke(props.quoteData.slice(5, 7))}, &nbsp;
              {props.quoteData.slice(0, 4)}
            </span>
            <div className="date-line-info__line">
              <svg
                width="9"
                height="1"
                viewBox="0 0 9 1"
            ...
            <span>{props.quoteData.slice(11, 16)}</span>
          </div>
          <div className="aeroflot-line-info">
            <span>{convertIdToCarrier(props.qouteAeroflot)}</span>
          </div>
        </div>
        <div className="flights-container-footer__item__secondary-info-wrapper">
          <div className="flights-container-footer__item__secondary-info">
            <div className="like-item">
              <svg
                ...
                viewBox="0 0 23 20"
                fill={
                  props.flightsFavorites.includes(props.quoteId)
                    ? "#F04393"
                    : "none"
                }
                xmlns="http://www.w3.org/2000/svg"
            ...
            <div className="price-info">
              <span>Price:</span>
              <span>
                {props.quotePrice}&nbsp;{props.currenciesSymbol}
              </span>
            </div>
          </div>
        </div>
...
export default FlightsContainerFooterItem;
